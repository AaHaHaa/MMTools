...
if use_gpu
    ...
    if ~exist(fullfile(cuda_dir_path,'decompose_into_modes_Cartesian_basis.ptx'), 'file')
        if ispc
            ...
            system(['nvcc -ccbin clang-3.8 -ptx "', fullfile(cuda_dir_path,'decompose_into_modes_Cartesian_basis.cu'), '" --output-file "', fullfile(cuda_dir_path,'decompose_into_modes_Cartesian_basis.ptx') '"']);
        end
    end
