...
if any(n2_prefactor(:)) % Compute the nonlinearity only when n2 isn't zero
    ...
    if sim.include_Raman
        if sim.scalar
            Ra = dt*fft(haw.*ifft(abs(E_tr_wNoise).^2));
            
            nonlinear_tr = Kerr + 2*Ra.*E_tr_wNoise;
        else
            Ra = dt*fft(haw.*ifft(sum(abs(E_tr_wNoise).^2,5)));

            if isempty(hbw)
                nonlinear_tr = Kerr + 2*Ra.*E_tr_wNoise;
            else % polarized field with an anisotropic Raman
                Rb = dt*fft(hbw.*ifft(E_tr_wNoise.*permute(conj(E_tr_wNoise),[1,2,3,4,6,5]) + conj(E_tr_wNoise).*permute(E_tr_wNoise,[1,2,3,4,6,5])));

                nonlinear_tr = Kerr + ( 2*Ra.*E_tr_wNoise + sum(Rb.*permute(E_tr_wNoise,[1,2,3,4,6,5]),6) );
            end
        end
    else
        nonlinear_tr = Kerr;
