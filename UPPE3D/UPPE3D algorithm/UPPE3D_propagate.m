...
if Nt == 1 % CW case
    ...
    omegas = 2*pi*ifftshift(linspace(-floor(Nt/2), floor((Nt-1)/2), Nt))'/(Nt*dt); % in 1/ps, in the order that the ifft gives
...
E_tr_noise = shot_noise(sim,...
                        Nt,dt,...
                        size(initial_condition.field),...
                           dx,dy);
if sim.gpu_yes
