...
if ~isequal(gain_rate_eqn.pump_direction,'co') || gain_rate_eqn.include_ASE
    ...
    for i = 2:gain_rate_eqn.max_iterations
        ...
        if i == gain_rate_eqn.max_iterations
            warning('SteppingCaller_rategain_rmc:NotConvergedError',...
...
signal_fields_out          = fft(cell2mat(signal_fields(:,:,saved_zPoints)),[],1);
%signal_fields_backward_out = fft(cell2mat(signal_fields_backward(:,:,saved_zPoints)),[],1);
...
switch direction
    case 'forward'
        [signal_fields,Power_pump_forward,Power_ASE_forward] = initialization('forward',...
                                                                              gain_rate_eqn,...
                                                                              1,num_segments,...
                                                                              Nt,num_modes,...
                                                                              num_zPoints,...
    ...
    case 'backward'
        ...
        [Power_pump_backward,Power_ASE_backward] = initialization('backward',...
                                                                  gain_rate_eqn,...
...
if sim.progress_bar
    ...
    elseif ~ischar(sim.progress_bar_name)
        error('SteppingCaller_rategain_rmc:ProgressBarNameError',...
...
for ii = 2:num_zPoints
    ...
    if sim.progress_bar && getappdata(h_progress_bar,'canceling')
        error('SteppingCaller_rategain_rmc:ProgressBarBreak',...
    ...
    if isequal(direction,'forward')
        ...
        if any(any(isnan(last_signal_fields))) %any(isnan(last_signal_fields),'all')
            error('GMMNLSE_propagate:NaNError',...
                'NaN field encountered, aborting.\nPossible reason is that the nonlinear length is too close to the large step size, dz*M for MPA or dz for split-step.');
        end
