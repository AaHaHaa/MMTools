__global__ void recompose_into_space(double2* full_field_xyw,
    ...
    for (unsigned int mi = 0; mi<sM; mi++) {
        full_field_xyw[thread_idx].x = full_field_xyw[thread_idx].x + mode_space_profiles_xym[XYi+sXY*mi]*field_wm[wi+sW*mi].x/norm_spatial_modes[mi];
        full_field_xyw[thread_idx].y = full_field_xyw[thread_idx].y + mode_space_profiles_xym[XYi+sXY*mi]*field_wm[wi+sW*mi].y/norm_spatial_modes[mi];
    }
