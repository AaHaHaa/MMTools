...
if isequal(field_type,'ASE') % ASE
    if isscalar
        ...
        E_photon = 2*E_photon*1e12*sponASE_spatial_modes*abs(cs); % W/THz; 1e12 is to transform Hz into THz (J=W/Hz)
    else
        E_photon = E_photon*1e12*sponASE_spatial_modes*abs(cs); % W/THz; 1e12 is to transform Hz into THz (J=W/Hz)
    end
end
