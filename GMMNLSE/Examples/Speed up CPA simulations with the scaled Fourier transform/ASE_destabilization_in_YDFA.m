...
% transformation based on scaled Fourier transform. Narrowband
% transformation is a newly-proposed technique and is applied here to speed
% up simulations.
%
...
[~,chirped_pulse] = func.General( chirped_tfwhm,omega,ifft(input_field.fields),chirp_sign );
input_field.fields = chirped_pulse;

%% Gain parameters
% Precompute some parameters related to the gain to save the computational time
% Check "gain_info.m" for details.
cs = 8; % this scaled factor for the scaled Fourier transform needs to be transfered into gain_info()
sim.cs.cs = cs;

gain_rate_eqn_with_ASE = gain_info( fiber,sim,gain_rate_eqn,ifftshift(lambda,1) );

% Prepare for the one without ASE
gain_rate_eqn.ignore_ASE = true;
gain_rate_eqn_without_ASE = gain_info( fiber,sim,gain_rate_eqn,ifftshift(lambda,1) );
...
% Spectrum
figure;
plot(lambda,abs(fftshift(ifft(output_with_ASE.fields(:,:,end),[],1),1)).^2.*factor*factor_correct_unit,'linewidth',2,'Color','r');
...
% Spectrum
figure;
plot(lambda,abs(fftshift(ifft(output_without_ASE.fields(:,:,end),[],1),1)).^2.*factor*factor_correct_unit,'linewidth',2,'Color','r');
xlabel('Wavelength (nm)');
ylabel('PSD (nJ/nm)');
xlim([850 1350]);
set(gca,'fontsize',20);
