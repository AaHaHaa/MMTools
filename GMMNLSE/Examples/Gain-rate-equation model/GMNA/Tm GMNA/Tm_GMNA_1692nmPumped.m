% This code solves the 6-um-core Tm-doped gain-managed fiber amplifier 
% (GMNA).
...
gain_rate_eqn.core_diameter = 2.2; % um
gain_rate_eqn.cladding_diameter = 2.2; % um
gain_rate_eqn.core_NA = 0.36;
gain_rate_eqn.absorption_wavelength_to_get_N_total = 793; % nm
gain_rate_eqn.absorption_to_get_N_total = 1000; % dB/m
gain_rate_eqn.pump_wavelength = 1692; % nm
...
gain_rate_eqn.t_rep = 1/30e6; % Assume 30 MHz here; s; the time required to finish a roundtrip (the inverse repetition rate of the pulse)
...
%gain_rate_eqn.kijkl = zeros(size(gain_rate_eqn.kijkl));
...
[Strehl_ratio,dechirped_FWHM,transform_limited_FWHM,peak_power] = analyze_field( t,f,prop_output.fields(:,:,end),'Treacy-t',pi/6,1e-3/300 );

func = analyze_sim;
pump_plot.forward  = prop_output.Power.pump.forward;
pump_plot.backward = prop_output.Power.pump.backward;
fig_gain = func.analyze_gain(prop_output.z,[],pump_plot,prop_output.population);

Ntmp = cat(4,1-sum(prop_output.population,4),prop_output.population);
Ntmp = permute(Ntmp,[1,2,3,5,6,7,8,4])*gain_rate_eqn.N_total;
