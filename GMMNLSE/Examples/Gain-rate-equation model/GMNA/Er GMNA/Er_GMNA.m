% This code solves the 5.2-um-core Er-doped gain-managed fiber amplifier 
% (GMNA). The small 5.2-um core diameter creates normal dispersion around
% 1550 nm for the GMN state to occur.
% The fiber is EDF50PM from OFS.
...
fiber_Gain.MFD = 5.2; % um; mode-field diameter
fiber_Gain.L0 = 5; % m; fiber length
...
gain_rate_eqn.cladding_diameter = 4; % um; this is made the same as the core_diameter to simulate core pumping of EDF50PM
gain_rate_eqn.core_NA = 0.27;
gain_rate_eqn.absorption_wavelength_to_get_N_total = 1530; % nm
gain_rate_eqn.absorption_to_get_N_total = 50; % dB/m
gain_rate_eqn.pump_wavelength = 976; % nm
gain_rate_eqn.copump_power = 0.01; % W
...
gain_rate_eqn.t_rep = 1/250e3; % Assume 250 kHz here; s; the time required to finish a roundtrip (the inverse repetition rate of the pulse)
...
%% Load fiber betas from OFS measurement data
...
OFS = load('beta_EDF50PM.mat');
fiber_Gain.betas = interp1(OFS.wavelength*1e9,OFS.beta0,lambda,'pchip','extrap');
...
[Strehl_ratio,dechirped_FWHM,transform_limited_FWHM,peak_power] = analyze_field( t,f,prop_output.fields(:,:,end),'Treacy-t',pi/6,1e-3/600 );
