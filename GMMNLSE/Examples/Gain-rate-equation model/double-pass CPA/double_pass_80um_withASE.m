...
gain_rate_eqn.t_rep = 1/100e3; % Assume 4 MHz here; s; the time required to finish a roundtrip (the inverse repetition rate of the pulse)
...
% Gain fiber
sim.gain_model = 2;
fiber.L0 = 0.95; % the length of the gain fiber
fiber.MFD = 64;
...
while iterations_num < max_iterations
    ...
    for i = 1:2
        ...
        if i == 1
            ...
            input_pulse{2}.Power.ASE.forward = prop_output.Power.ASE.forward(:,:,end);
            input_pulse{2}.Power.ASE.backward = prop_output.Power.ASE.backward(:,:,1);
        else % i == 2
            input_pulse{1}.Power.ASE.forward = zeros(Nt,1);
            input_pulse{1}.Power.ASE.backward = prop_output.Power.ASE.backward(:,:,1);
        end
...
save('double_pass_withASE.mat','-v7.3');