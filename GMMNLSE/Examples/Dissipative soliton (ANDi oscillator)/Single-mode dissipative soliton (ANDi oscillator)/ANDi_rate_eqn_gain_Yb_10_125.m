...
sim10 = sim; % for the passive fibers
sim10.progress_bar_name = 'SMF (10um)';
sim10.save_period = 0.1; % m

fiber10.MFD = 10; % um
[fiber10,sim10] = load_default_GMMNLSE_propagate(fiber10,sim10,'single_mode'); % for the passive fibers
...
% ----------------------------------- All ---------------------------------
% -------------------------------------------------------------------------
fiber_cavity = [fiber_a fiber_Gain fiber_b];
sim_cavity = [sim10 sim_Gain sim10];

%% Setup general cavity parameters
max_rt = 500; % maximum roundtrips (in case it doesn't converge)
Nt = 2^12; % the number of points
time_window = 100; % ps
dt = time_window/Nt;
...
while rt_num < max_rt
    ...
    % prop_output = saturable_absorber_action_NPE(prop_output, 0.3,0.2,0,400);
