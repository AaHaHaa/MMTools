% This code runs the multimode soliton oscillator with
% an OM4 multimode fiber (with 50-um-diameter core) and a passive/Er-doped 
% 15-um LMA fiber. The gain fiber is modeled with the rate equations. I put
% the OM4 fiber after the few-mode 15-um fiber to enhance the multimode
...
%
% I use an artificially-created Er-doped double-cladding multimode fiber as
% the gain fiber.
%
...
%                                                                                |
%                                                                                |
%   --> 15-um passive fiber --> 15-um gain fiber --> OM4 passive fiber --> output coupler --
%   |                                                                            |
%   -------- spectral filter <-- spatial filter <-- saturable absorber <----------
%
% When the pulse couples back into the 15-um fiber, we assume that there is
% a perfect imaging system such that the end-facet of the OM4 fiber is
% imaged to the 15-um fiber. With this imaging system, the coupling matrix
...
num_modes_OM4 = 6;
fiber_OM4.MM_folder = '../../../Fibers/OM4_wavelength1550nm/';
fiber_OM4.betas_filename = 'betas.mat';
fiber_OM4.S_tensors_filename = sprintf('S_tensors_%umodes.mat',num_modes_OM4);

% For 15-um 6-mode/few-mode gain fiber
num_modes_15um = 6; % 6 modes in total in the generated mode profiles from the mode solver in "BuildFiber/"
num_modes_15um_considered = 3; % this code is for demonstration, so I consider 3 modes for the longer fibers
fiber_15um.MM_folder = '../../../Fibers/step_wavelength1550nm_diameter15um/';
fiber_15um.betas_filename = 'betas.mat';
fiber_15um.S_tensors_filename = sprintf('S_tensors_%umodes.mat',num_modes_15um);
...
gain_rate_eqn.core_diameter = 15; % um
...
gain_rate_eqn.absorption_to_get_N_total = 3; % dB/m
gain_rate_eqn.pump_wavelength = 976; % nm
gain_rate_eqn.copump_power = 13; % W
gain_rate_eqn.counterpump_power = 0; % W
gain_rate_eqn.downsampling_factor = 2; % an integer; downsample the eigenmode profiles to run faster
...
% -------------------------------------------------------------------------
% Passive fiber (15-um fiber)
% -------------------------------------------------------------------------
sim_15um = sim; % this loads the above general parameters
sim_15um.progress_bar_name = 'Fiber before gain (15um)';
sim_15um.save_period = 0.1; % m
fiber_15um.L0 = 10; % m; fiber length
sim_15um.midx = 1:num_modes_15um_considered; % only the first "num_modes_15um_considered" modes are considered
[fiber_15um,sim_15um] = load_default_GMMNLSE_propagate(fiber_15um,sim_15um,'multimode'); % for the passive fiber before the gain fiber

% -------------------------------------------------------------------------
% Gain fiber (15-um Er-doped double-cladding fiber)
% -------------------------------------------------------------------------
sim_Gain = sim; % this loads the above general parameters
sim_Gain.gain_model = 2; % use the rate-eqn-gain model
sim_Gain.progress_bar_name = 'Gain (15um)';
sim_Gain.save_period = 0.1; % m
sim_Gain.midx = 1:num_modes_15um_considered; % only the first "num_modes_15um_considered" modes are considered
fiber_Gain.MM_folder = fiber_15um.MM_folder;
fiber_Gain.betas_filename = fiber_15um.betas_filename;
fiber_Gain.S_tensors_filename = fiber_15um.S_tensors_filename;
fiber_Gain.L0 = 1; % m; fiber length
...
Aeff = 1/fiber_15um.SR(1); % assume 15-um fundamental mode for calculating "saturation_'intensity'=saturation_power/Aeff"
...
spectral_filter = struct('bw',10, ... % bandwidth (nm)
...
L0 = fiber_15um.L0 + fiber_Gain.L0 + fiber_OM4.L0; % total fiber length
save_num = int64(fiber_15um.L0/sim_15um.save_period + fiber_Gain.L0/sim_Gain.save_period + fiber_OM4.L0/sim_OM4.save_period - 1); % the total number of fields to save
...
for midx = 1:num_modes_OM4
    data = load(sprintf('%smode%uwavelength15500.mat',fiber_OM4.MM_folder,midx));
...
% Load 15-um-fiber mode profiles
Nx_15um = 400; % this is known in advance (=length(data.x) below)
mode_profiles_15um = zeros(Nx_15um,Nx_15um,num_modes_15um_considered);
for midx = 1:num_modes_15um_considered
    data = load(sprintf('%smode%uwavelength15500.mat',fiber_15um.MM_folder,midx));
    mode_profiles_15um(:,:,midx) = data.phi; % 1/sqrt(um)
end
% Normalize the mode fields for later computations
norm_mode_profiles_15um = sqrt(sum(sum(mode_profiles_15um.^2,1),2))*mean(diff(data.x*1e-6));
mode_profiles_15um = downsample2(mode_profiles_15um./norm_mode_profiles_15um,Nx_15um/Nx_OM4*downsample_ratio); % 1/sqrt(m)
mode_profiles_15um_x = downsample(data.x,Nx_15um/Nx_OM4*downsample_ratio); % um

% Coupling matrix from OM4 to the 15-um fiber
fiber_coupling_matrix_OM42Gain = calc_mode_coupling_matrix(mode_profiles_OM4_x,  mode_profiles_OM4,...
                                                           mode_profiles_15um_x, mode_profiles_15um);
% Coupling matrix from the 15-um fiber to OM4
fiber_coupling_matrix_Gain2OM4 = calc_mode_coupling_matrix(mode_profiles_15um_x, mode_profiles_15um,...
                                                           mode_profiles_OM4_x,  mode_profiles_OM4);

% Offset in offset splicing between 15-um and OM4 fibers
offset = 5; % um
...
ratio = abs(mode_profiles_15um_x(1)/mode_profiles_OM4_x(1));
resized_mode_profiles_15um = spatial_field_resizing(mode_profiles_15um, ratio);
resized_mode_profiles_15um_x = mode_profiles_15um_x/ratio;
...
while rt_num < max_rt
    ...
    % imaged to 15-um fiber.
    prop_output.fields = (fiber_coupling_matrix_OM42Gain*prop_output.fields(:,:,end).').';

    % Fiber propagation
    prop_output = GMMNLSE_propagate(fiber_15um, prop_output, sim_15um);
    ...
    % (1) offset splicing
    % The 15-um field is converted into the spatial domain and then converted into the mode domain of the OM4 fiber
    full_field_txy = recompose_into_space(sim_Gain.gpu_yes, resized_mode_profiles_15um, prop_output.fields(:,:,end), sim_Gain.cuda_dir_path);
    % Offset the field of offset splicing
    full_field_txy = circshift(full_field_txy,ceil(offset/mean(diff(resized_mode_profiles_15um_x))),2);
...
[Strehl_ratio,dechirped_FWHM,transform_limited_FWHM,peak_power] = analyze_field( t,f,output_field(:,1,end),'Treacy-t',pi/6,1e-3/600,true );

%% Plot the output spatial profile
full_field_txy = recompose_into_space(sim_Gain.gpu_yes, mode_profiles_15um, output_field(:,:,end), sim_Gain.cuda_dir_path);

figure;
pcolor(mode_profiles_15um_x,mode_profiles_15um_x,squeeze(sum(abs(full_field_txy).^2,1)));
