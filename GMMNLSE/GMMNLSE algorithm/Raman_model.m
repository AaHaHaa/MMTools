...
switch fiber.material
    case {'silica','fused silica'}
        switch sim.Raman_model
            ...
            case 2 % isotropic and anisotropic Raman
                ...
                % For scalar fields, anisotropic Raman is incorporated into ha.
                if sim.scalar
    ...
    case 'chalcogenide'
        switch sim.Raman_model
            case {0,1} % no Raman and isotropic Raman
                ...
                ha = ((t1^2+t2^2)/(t1*t2^2)).*exp(-t_shifted/t2).*sin(t_shifted/t1);
            otherwise
                error('Raman_model:modelError',...
                      'Anisotropic Raman model hasn''t been implemented for chalcogenide fibers yet.');
        end
...
% isotropic Raman
haw = ifft(ha)*Nt*dt; % The factor of N is needed and used later in the convolution theorem because of how ifft is defined.
...
% anisotropic Raman
if ~isempty(hb)
    hbw = ifft(hb)*Nt*dt; % The factor of N is needed and used later in the convolution theorem because of how ifft is defined.
                      % The convolution theorem is PQ=F[(p*q)]/N, where (p*q) is the discrete-form convolution, for discrete Fourier transforms.
                      % Normal (integral-form) convolution is p*q=(p*q)*dt.
    if sim.gpu_yes
        hbw = gpuArray(hbw);
