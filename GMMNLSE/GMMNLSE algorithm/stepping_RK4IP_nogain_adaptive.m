function [A1w,a5,...
          opt_dz,success] = stepping_RK4IP_nogain_adaptive(A0w, dummy_dt,...
                                                           sim, n2_prefactor,...
...
%
%    dummy_G - gain term (Nt,1); but this isn't used in this function
    ...
    for midx1 = 1:num_modes
        ...
        if sim.include_Raman
            ...
            for midx2 = 1:num_modes
                ...
                Ra_sponRS(:, midx1, midx2) = sum(permute(SRa_info.SRa(nz_midx),[2 1]).*Ra_mn_sponRS(:, idx),2); % spontaneous Raman scattering
            end
            % Rb
            if ~isempty(hbw)
                for midx2 = 1:num_modes
                    ...
                    Rb_sponRS(:, midx1, midx2) = sum(permute(SRb_info.SRb(nz_midx),[2 1]).*Rb_mn_sponRS(:, idx),2); % spontaneous Raman scattering
                end
            end
        end
    end
...
if sim.include_Raman
    Ra = fft(haw.*ifft(Ra)); % stimulated Raman scattering
    Ra_sponRS = fft(haw.*ifft(Ra_sponRS).*sponRS_prefactor{2}); % spontaneous Raman scattering
    
    if ~isempty(hbw) % polarized fields with an anisotropic Raman
        Rb = fft(hbw.*ifft(Rb)); % stimulated Raman scattering
        Rb_sponRS = fft(hbw.*ifft(Rb_sponRS).*sponRS_prefactor{2}); % spontaneous Raman scattering
    end
    
    if isempty(hbw)
        nonlinear = Kerr + sum((Ra+Ra_sponRS).*permute(At,[1 3 2]) ,3);
    else % polarized fields with an anisotropic Raman
        nonlinear = Kerr + sum((Ra+Rb+Ra_sponRS+Rb_sponRS).*permute(At,[1 3 2]) ,3);
    end
else
    nonlinear = Kerr;
end

% Now eveyerthing has been summed into "nonlinear", so transform into the
