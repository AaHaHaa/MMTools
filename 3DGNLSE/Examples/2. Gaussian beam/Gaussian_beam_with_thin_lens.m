...
prop_output1 = GNLSE3D_propagate(fiber,initial_condition,sim);
% lens
focal_length = 0.5; % m
Ef_out = add_thin_lens_phase(ifft(prop_output1.field,[],1),prop_output1.dx,prop_output1.dy,fftshift(lambda,1)/1e9,focal_length);
% free space after the lens
initial_condition2 = prop_output1; initial_condition2.field = fft(Ef_out(:,:,:,end),[],1);
prop_output2 = GNLSE3D_propagate(fiber,initial_condition2,sim);
